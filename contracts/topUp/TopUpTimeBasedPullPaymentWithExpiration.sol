...
/// @dev - Top Up Billing model - Total Limit & Time-based limit & Expiration Time
...
/// The customer can increase/decrease the top up and the time based limit at any point and the expiration time as well.
contract TopUpTimeBasedPullPaymentWithExpiration is PayableOwnable {
    ...
    isValidExpirationTimestamp(_numbers[7])
    {
        ...
        require(_numbers[7] <= OVERFLOW_LIMITER_NUMBER, "Invalid number - Must be lower than the overflow limit.");
        ...
        pullPayments[paymentIDs[0]].expirationTimestamp = _numbers[7];
    ...
    ///     should be valid in terms of whether it can be executed i.e.
    ///     it is within the total and time based limits and before the expiration time.
