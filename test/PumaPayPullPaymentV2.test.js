...
contract('PumaPay Pull Payment V2 Contract', async (accounts) => {
  ...
  describe('Register Single Pull Payment', async () => {
    ...
    it('should add the pull payment for the beneficiary in the active payments array', async () => {
      const signature = await signRegistrationV2(singlePullPayment, CLIENT_ONE_PRIVATE_KEY);
  ...
  describe('Register Recurring Pull Payment', async () => {
    ...
    it('should add the pull payment for the beneficiary in the active payments array', async () => {
      const signature = await signRegistrationV2(recurringPullPayment, CLIENT_TWO_PRIVATE_KEY);
      const sigVRS = await getVRS(signature);
      ...
      const ethDate = await currentBlockTime();
      const activePaymentInArray = await pumaPayPullPayment.pullPayments(clientTwo, paymentExecutorTwo);
  ...
  describe('Register Recurring Pull Payment with Initial Payment', async () => {
    ...
    it('should add the pull payment for the beneficiary in the active payments array', async () => {
      const signature = await signRegistrationV2(recurringWithInitialAmount, CLIENT_THREE_PRIVATE_KEY);
      const sigVRS = await getVRS(signature);
      ...
      const ethDate = await currentBlockTime();
      const activePaymentInArray = await pumaPayPullPayment.pullPayments(recurringWithInitialAmount.client, paymentExecutorThree);
  ...
  describe('Register Recurring Pull Payment with Free Trial', async () => {
    ...
    it('should add the pull payment for the beneficiary in the active payments array', async () => {
      const signature = await signRegistrationV2(recurringWithFreeTrial, CLIENT_THREE_PRIVATE_KEY);
      const sigVRS = await getVRS(signature);
      ...
      const activePaymentInArray = await pumaPayPullPayment.pullPayments(recurringWithFreeTrial.client, paymentExecutorThree);
  ...
  describe('Register Recurring Pull Payment with Paid Trial', async () => {
    ...
    it('should add the pull payment for the beneficiary in the active payments array', async () => {
      const signature = await signRegistrationV2(recurringWithPaidTrial, CLIENT_THREE_PRIVATE_KEY);
      const sigVRS = await getVRS(signature);
      ...
      const ethDate = await currentBlockTime();
      const activePaymentInArray = await pumaPayPullPayment.pullPayments(recurringWithPaidTrial.client, paymentExecutorThree);
  ...
  describe('Cancel Recurring Pull Payment', async () => {
    ...
    it('should set the cancel date of the pull payment for the paymentExecutorOne to NOW', async () => {
      const signature = await signDeletionV2(recurringPullPayment.paymentID, recurringPullPayment.pullPaymentExecutorAddress, CLIENT_TWO_PRIVATE_KEY);
      const sigVRS = await getVRS(signature);
